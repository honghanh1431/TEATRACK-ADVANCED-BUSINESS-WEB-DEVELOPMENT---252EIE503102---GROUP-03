...
export class Admin implements OnInit, AfterViewInit, OnDestroy {
      ...
      next: (res) => {
        ...
        this.http.get<Order[]>('/data/orders.json').subscribe({
          next: (fromJsonRaw) => {
            ...
            // Ưu tiên: Database > Storage > JSON
            [...fromDb, ...fromStorage, ...fromJson].forEach((o) => {
              // MongoDB dùng _id, dữ liệu cũ dùng orderId hoặc id
              const id = String(o._id || o.id || o.orderId || '').trim();
              if (id && !ids.has(id)) {
                ids.add(id);
                // Đảm bảo o có property id cho đồng bộ giao diện
  ...
  private fallbackFetchOrders(): void {
    this.http.get<Order[]>('/data/orders.json').subscribe({
      next: (data) => {
  ...
  fetchProducts(): void {
    this.http.get<Product[]>(this.PRODUCTS_API).subscribe({
      ...
      error: (err) => {
        ...
        // Fallback to local data or storage if needed, but primarily use API
        this.http.get<Product[]>('/data/products.json').subscribe({
          next: (localData) => {
            this.PRODUCTS_ALL = localData;
            this.applyProductsLoaded();
          },
          error: () => {
            this.PRODUCTS_ALL = [];
  ...
  private initAddProductForm(): void {
    ...
    form.addEventListener('submit', (e) => {
      ...
      this.http.post<Product[]>(this.PRODUCTS_API, data).subscribe({
        next: (updatedList) => {
          this.PRODUCTS_ALL = updatedList;
          this.applyFilters();
          this.updateProductsStats();
  ...
  private initEditProductForm(): void {
    ...
    form.addEventListener('submit', (e) => {
      ...
      this.http.put<Product[]>(`${this.PRODUCTS_API}/${productId}`, updatedProduct).subscribe({
        next: (updatedList) => {
          this.PRODUCTS_ALL = updatedList;
          this.applyFilters();
  ...
  confirmDeleteProduct(): void {
    ...
    this.http.delete<Product[]>(`${this.PRODUCTS_API}/${this.pendingDeleteProductId}`).subscribe({
      next: (updatedList) => {
        this.PRODUCTS_ALL = updatedList;
        this.applyFilters();
        this.updateProductsStats();
